...
xmlns:tsoFn="http://www.tso.co.uk/assets/namespaces/functions"
...
exclude-result-prefixes="tso atom">
	...
	<!-- text generation for the participation maodel -->
...
<!-- ##############################################  -->
	...
	<xsl:template match="leg:Span[@markup = 'Gate'][not(ancestor::leg:Footnote)]">
		<fo:inline color="{$g_strLinkColor}">
			<xsl:attribute name="background-color">
				<xsl:text>#ffff00</xsl:text>
			</xsl:attribute>
			<xsl:apply-templates/>
	...
	<xsl:template match="leg:AppendText/leg:FootnoteRef" priority="20">
		<fo:block>
			<xsl:next-match/>
		</fo:block>
	</xsl:template>
	...
	<xsl:template match="leg:FootnoteRef">
		...
		<xsl:choose>
			...
			<!-- there are rare occurances where the footnote is not placed as an inline element - in which case we need to add an fo:block to make certain that it will not be a child of fo:flow
			sucb an instance occurs in uksi/2011/2053/made/revision  -->
	...
	<xsl:template name="makeFootnote">
		<xsl:param name="strFootnoteRef"/>
		<xsl:param name="intFootnoteNumber"/>
		<xsl:param name="booTableRef"/>
		<fo:footnote>
			<xsl:choose>
				<xsl:when test="$booTableRef = true()">
					<xsl:variable name="nstTableFootnoteRefs" select="ancestor::xhtml:table/xhtml:tfoot//leg:Footnote" as="element(leg:Footnote)*"/>
	...
	<!-- ####################################################  -->
	<!-- templates for dealing with unapplied effects markup for participation model  -->

	<xsl:key name="effects" match="ukm:Effect | ukm:UndefinedEffect" use="@Ref" />

	<xsl:template match="leg:Span[key('effects',@id)]" priority="200">
		<fo:inline text-decoration="underline">
			<xsl:apply-templates/>
		</fo:inline>
	</xsl:template>	
		
	<xsl:template match="leg:Text[leg:Span[key('effects',@id)] | leg:Citation[key('effects',@id)] ]" priority="1001">
		<xsl:next-match/>
		<xsl:apply-templates select="leg:Span[key('effects',@id)] | leg:Citation[key('effects',@id)]"   mode="effect"/>
	</xsl:template>		

	
	<xsl:template match="leg:Span[key('effects',@id)] |  leg:Citation[key('effects',@id)]"  mode="effect">
		<!-- there may be more than one effect associated with the reference  -->
		<xsl:variable name="effect" as="element()*" select="key('effects', @id)" />	
		<xsl:for-each select="$effect">
			<!-- the PDF version do not want 'coming into force' effects  -->
			<xsl:if test="./@Type != 'coming into force'">
				<fo:block-container margin-right="0pt">
					<fo:table margin-top="3pt" table-layout="fixed" margin-left="0pt" margin-right="0pt">
						...
						<fo:table-body margin-left="0pt" margin-right="0pt">
							<fo:table-row>
								...
								<fo:table-cell padding="3pt" border="solid 0.5pt #ff0000" margin="3pt" text-align="left" background-color="#FFD3DA">
									<fo:block>
										<xsl:apply-templates select="." mode="effect"/>
									</fo:block>
									<fo:block font-size="8pt">
										<fo:block>
											<xsl:text>Affected Legislation: </xsl:text>
											<xsl:choose>
												...
												<xsl:when test="@AffectedClass and @AffectedYear and @AffectedNumber">
													<xsl:value-of select="tsoFn:toesReference(@AffectedClass,@AffectedYear,@AffectedNumber)" />
												</xsl:when>
												<xsl:otherwise>
										...
										<fo:block>
											<xsl:text>Affected Provision: </xsl:text>
											<xsl:value-of select="@AffectedProvisions"/>
										</fo:block>
										<fo:block>
											<xsl:text>Type of Effect: </xsl:text>
	...
	<xsl:template match="leg:ExplanatoryNotes" priority="20">
		<xsl:next-match/>
		<xsl:apply-templates select="." mode="ProcessAnnotations"/>
	</xsl:template>
			...
			<fo:block  margin-left="0pt" margin-right="0pt" font-size="10pt" border="0.75pt #c7c7c7 solid" padding="6pt" color="black" space-before="8pt"   >
				...
				<xsl:if test="not($higherLevelComments/*)">
					...
					<xsl:for-each-group select="$showComments" group-by="@Type">
						...
						<xsl:if test="not($documentType = ('NorthernIrelandAct' , 'NorthernIrelandOrderInCouncil' , 'NorthernIrelandStatutoryRule', 'NorthernIrelandAssemblyMeasure', 'NorthernIrelandParliamentAct', 'NorthernIrelandStatutoryRuleOrOrder') and $documentYear &lt; 2006 and $groupType = 'F' )">
...
<!-- Suppress repealed content -->
	...
	<xsl:template match="leg:Group[.//leg:Repeal] | leg:Part[.//leg:Repeal] | leg:Chapter[.//leg:Repeal] | 
		...
		<xsl:choose>
			<xsl:when test="$showRepeals or not(tso:isRepealed(.))">
				<xsl:next-match />		
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>. . . . . . . . . . . . . . . . . . . . . . . . . . .</xsl:text>		
	...
	<xsl:template match="leg:Strong[not(node())] | leg:Superior[not(node())]" priority="100"/>
