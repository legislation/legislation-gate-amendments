...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml"  version="2.0" 
	...
	<xsl:variable name="generalSearch" as="xs:boolean" select="not($paramsDoc/parameters/search-type = ('extent', 'point-in-time','draft-legislation','impacts','sparql') ) and not($isVersionSpecified)" />
	...
	<xsl:variable name="sparqlSearch" as="xs:boolean" select="$paramsDoc/parameters/search-type = 'sparql'" />
	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<xsl:choose>
					<xsl:when test="$sparqlSearch">
						<script type="text/javascript" src="/scripts/flint/lib/codemirror.js">//</script>
						<script type="text/javascript" src="/scripts/flint/sparql10querymode_ll1.js">//</script>
						<script type="text/javascript" src="/scripts/flint/sparql11querymode_ll1.js">//</script>
						<script type="text/javascript" src="/scripts/flint/sparql11updatemode_ll1.js">//</script>
						<script type="text/javascript" src="/scripts/flint/flint-editor.js">//</script>
						<script type="text/javascript" src="/scripts/advancedsearch/flint.js">//</script>
						<link rel="stylesheet" href="/scripts/flint/lib/codemirror.css"/>
						<link rel="stylesheet" href="/styles/flint/sparqlcolors.css"/>
						<link rel="stylesheet" href="/styles/flint/docs.css"/>
						<link rel="stylesheet" href="/styles/advancedsearch/flint.css"/>
					</xsl:when>
					<xsl:otherwise>
			...
			<body lang="en" xml:lang="en" dir="ltr" id="search" class="intro">
				<div id="layout2">
				<xsl:attribute name="class"><xsl:if test="$sparqlSearch">searchTool</xsl:if></xsl:attribute>
					<!-- adding quick search  -->
					...
					<div id="tools">
						...
						<div id="links">
							...
							<a href="/search/sparql">
								<xsl:if test="$sparqlSearch">
									<xsl:attribute name="class">current</xsl:attribute>
								</xsl:if>
					...
					<!--/#content-->
							...
							<!-- Due to width of Flint this section is brought out of div content -->
					<xsl:if test="$sparqlSearch">
							<xsl:call-template name="SPARQLsearch" />
					</xsl:if>
				<!--/#layout2-->
			</body>
		</html>
	...
	<xsl:template name="SPARQLsearch">
		<div id="flint">
		<h2>SPARQL search</h2>
			<div id="flint-fallback">
	...
	LIMIT 20
					...
					<div class="submit">
						<button class="userFunctionalElement" id="contentSearchSubmit" type="submit">SPARQL Search</button>
					</div>
				</form>
			</div>
		</div>
