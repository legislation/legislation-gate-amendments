...
exclude-result-prefixes="leg ukm math msxsl dc dct ukm fo xsl svg atom xhtml tso xs err">
...
<!-- Store metadata -->
<xsl:variable name="g_ndsMetadata" select="/(leg:Legislation|leg:Fragment)/ukm:Metadata"/>
...
<!-- Store margin notes -->
<xsl:variable name="g_ndsMarginNotes" select="/(leg:Legislation|leg:Fragment)/leg:MarginNotes/leg:MarginNote"/>

<!-- Store versions -->
<xsl:variable name="g_ndsVersions" select="/(leg:Legislation|leg:Fragment)/leg:Versions/leg:Version"/>
...
<xsl:template match="xhtml:head/xhtml:title">
	<title>
		...
		<xsl:choose>
			<xsl:when test="$g_ndsMainDoc/(leg:Legislation|leg:Fragment)/ukm:Metadata/dc:title">
				<xsl:value-of select="$g_ndsMainDoc/(leg:Legislation|leg:Fragment)/ukm:Metadata/dc:title" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$g_ndsMainDoc/(leg:Legislation|leg:Fragment)/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata)/ukm:Year/@Value" />
			</xsl:otherwise>
		</xsl:choose>
		<xsl:choose>
	...
	<style type="text/css" media="screen, print">
		...
		<xsl:choose>
			...
			<xsl:when test="$g_ndsMainDoc//leg:Primary and not($g_strDocumentType = $g_strSecondary)">
				<xsl:text>@import "</xsl:text>
				<xsl:value-of select="$g_strStylesPath"/>
				<xsl:text>primarylegislation.css";&#xA;</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>@import "</xsl:text>
				<xsl:value-of select="$g_strStylesPath"/>
				...
				<xsl:if test="$g_strDocumentMainType = 'NorthernIrelandAct'">
					<xsl:text>@import "</xsl:text>
					<xsl:value-of select="$g_strStylesPath"/>
					<xsl:text>NIlegislation.css";&#xA;</xsl:text>
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
		<xsl:text>@import "</xsl:text>
		<xsl:value-of select="$g_strStylesPath"/>
		<xsl:text>legislationOverwrites.css";</xsl:text>
	</style>
	<xsl:comment>[if IE 6]>
		&lt;style type="text/css">
			@import "<xsl:value-of select="$g_strStylesPath"/>IE/ie6LegAdditions.css";
		&lt;/style>
	&lt;![endif]</xsl:comment>
	<xsl:comment>[if IE 7]>
		&lt;style type="text/css">
			@import "<xsl:value-of select="$g_strStylesPath"/>IE/ie7LegAdditions.css";
		&lt;/style>
	&lt;![endif]</xsl:comment>
</xsl:template>
...
<xsl:template match="leg:Group | leg:Part | leg:Chapter | leg:Pblock | leg:PsubBlock | leg:Schedule" name="FuncProcessMajorHeading">
	...
	<xsl:apply-templates select="." mode="showInterweaves" />
...
<xsl:template match="*" mode="showInterweaves">
	<xsl:apply-templates select="." mode="showEN" />
	<xsl:apply-templates select="." mode="showTextualEffects" />
	<xsl:apply-templates select="." mode="showCommencementEffects" />
	<xsl:apply-templates select="." mode="showExtentEffects" />
</xsl:template>

<xsl:template match="*" mode="showEN" />
<xsl:template match="*" mode="showTextualEffects" />
<xsl:template match="*" mode="showCommencementEffects" />
<xsl:template match="*" mode="showExtentEffects" />
