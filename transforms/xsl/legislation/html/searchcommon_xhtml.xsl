...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml"  version="2.0" 
	...
	<xsl:template match="/">
		<html>
			...
			<body id="{$id}" class="{$class} searchResults" lang="en" dir="ltr" xml:lang="en">
				...
				<div id="layout2">
					...
					<xsl:call-template name="TSOOutputQuickSearch">
						<xsl:with-param name="IsParticipation" select="$supportParticipation"/>
					</xsl:call-template>
	...
	<xsl:template match="atom:feed" mode="links">
		...
		<xsl:variable name="link" as="xs:string" select="leg:GetLink(atom:link[@rel = 'first']/@href)"/>
			<div class="prevPagesNextNav pageNumNav">
				<ul>
	...
	<xsl:function name="leg:GetLink" as="xs:string">
		...
		<xsl:variable name="href" as="xs:string"
			select="replace($href, 'http://www.legislation.gov.uk', '')" />
		<xsl:variable name="href" as="xs:string"
	...
	<xsl:template match="atom:feed" mode="searchresults">
		<div id="content" class="results column-right">
	...
	<xsl:template name="AddBrowseStylesScripts">
		...
		<link rel="stylesheet" href="/styles/histogram.css" type="text/css"/>
