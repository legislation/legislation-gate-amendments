...
<!--
Due to a bug in Xmetal 5 we require all elements to have a namespace prefix rather than declaring the namespace on the element
-->
...
<xsl:template match="leg:Addition | leg:Repeal | leg:Substitution" priority="15">
	...
	<xsl:if test="((. is $first) and (not(ancestor::m:math or ancestor::math:math)))">		
		<xsl:element name="{concat('leg:',local-name(.))}">
			<xsl:copy-of select="@*" />
	...
	<xsl:element name="{string-join(($namespacePrefix,local-name(.)), ':')}">
		...
		<xsl:choose>
			...
			<xsl:when test="self::ukm:Metadata and ends-with(/leg:Fragment/@DocumentURI, '/revision')">
				<dc:source>https://<xsl:value-of select="$server-name" /><xsl:value-of select="if (exists($task) and $task = 'correct') then '/task/correct/legislation-provision/version/' else if (contains($request-uri, 'welsh-')) then (if (contains($request-uri, 'review')) then '/task/review/update/welsh-step/' else '/task/update/welsh-step/') else  if (contains($request-uri, 'review')) then '/task/review/update/step/' else '/task/update/step/'"/><xsl:value-of select="substring-before(substring-after(/leg:Fragment/@DocumentURI, 'http://www.legislation.gov.uk/'), '/revision')" />
				<xsl:value-of select="if (exists($version) and $task = 'correct' and $version = 'prospective') then '/prospective' else ()"/>
				<xsl:value-of select="if (exists($slug) and $task = 'correct') then concat('/',$slug) else ()"/>
				</dc:source>
			</xsl:when>
		</xsl:choose>
	</xsl:element>
...
<!-- resolve the namespaces so that the automation works in xmetal 5 -->

<xsl:template match="node()[not(self::err:* or self::leg:* or self::ukm:* or self::xhtml:*)] | @*">
	<xsl:copy>
...
<xsl:template match="ukm:*">
	<xsl:element name="{concat('ukm:',local-name(.))}">
		<xsl:apply-templates select="node() | @*"/>
	</xsl:element>
</xsl:template>

<xsl:template match="xhtml:*" priority="15">
	<xsl:element name="{concat('xhtml:',local-name(.))}">
		<xsl:apply-templates select="node() | @*"/>
	</xsl:element>
</xsl:template>

<xsl:template match="math:*" priority="15">
	<xsl:element name="{concat('math:',local-name(.))}">
		<xsl:apply-templates select="node() | @*"/>
	</xsl:element>
</xsl:template>
